...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
	...
	if(wall >= 0.8){following_wall = false;}
	...
	//if( std::isinf(door3) || std::isinf(door2 || thats_a_door) ){
	if( std::isinf(door3) && front > 0.6 && front <= 2.0 ){	
	...
	if(door4 > 630 && thats_a_door){thats_a_door = false;robot_move(FORWARD);}
	
	/*if( front < 0.3 && thats_a_door ){
		thats_a_door = false;
		//robot_move(STOP);
	...
	if(!following_wall && !thats_a_door){
		...
		if(door1 > 60 || door2 > 60){
			robot_move(GO_RIGHT);				
		}else{
			if(front > 0.3){robot_move(FORWARD);}
